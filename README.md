...
用户可在“设置 → AI 连接与模型”保存自己的 API Token 和 OpenAI 兼容中转站地址。
模型通过白名单下拉框选择，可用项为 `gpt-5.5`、`gpt-5.6-luna`、
`gpt-5.6-terra`、`gpt-5.6-sol` 和 `gpt-6-astra`，不接受任意模型 ID。
中转站地址留空时使用 OpenAI
官方 `https://api.openai.com/v1`；填写后改走该中转站，例如
`https://www.zenapi.org/v1`。Token 使用 AES-256-GCM 加密并仅在服务端解密，
浏览器只能看到是否已连接。

AI 是发布区内的可选动作：解析商品后可以选择“使用原图”或“AI 文案”，未点击时
不会调用模型。系统不提供
AI 图片生成，数据库也不保存草稿、图片、Base64 或商品内容。

## 评论、删除与定时发布

“管理评论”支持按当前主页的最近帖子读取评论和回复、以主页身份回复评论，以及
永久删除有权限管理的评论。“读取帖子”支持永久删除帖子。所有回复和删除操作均
要求浏览器会话、CSRF、明确确认和幂等键，并写入精简审计日志。

发布区支持 Meta 原生定时发布。用户以本地时间选择至少 10 分钟后、最多 180 天内
的时间，Page Center 立即把文本或公开 HTTPS 图片交给 Facebook 排程，并从
`scheduled_posts` 读取等待中的帖子。取消排程通过删除对应的 Meta 帖子完成，不使用
Vercel Cron，也不在数据库保存定时帖正文或图片。

## 公共主页详细信息

右侧信息卡显示当前公共主页的名称、Page ID、类别、Facebook 主页链接、网站、
联系电话、联系邮箱、地址、同步状态和最近同步时间。资料来自 Meta Pages API，
只保存短文本字段；点击卡片右上角的同步按钮可刷新 Facebook 主页中已填写的信息。
