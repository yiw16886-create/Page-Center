...
import { authenticate, clearSession, issueSession, requireCsrf, type AuthenticatedRequest } from "./auth.js";
import { metaConfig, readiness } from "./config.js";
import { completeAuthorization, createAuthorizationUrl, disconnect, listPosts, publishPost, status, verifyAuthorization } from "./meta-service.js";
...
router.post("/meta/disconnect", requireCsrf, asyncRoute(async (req: AuthenticatedRequest, res) => { await disconnect(req.actor!.id); res.json({ success: true }); }));
router.get("/pages/:pageId/posts", asyncRoute(async (req: AuthenticatedRequest, res) => res.json({ success: true, data: await listPosts(req.actor!.id, req.params.pageId, typeof req.query.after === "string" ? req.query.after : undefined) })));
router.post("/pages/:pageId/posts", requireCsrf, asyncRoute(async (req: AuthenticatedRequest, res) => {
  const result = await publishPost({ userId: req.actor!.id, pageId: req.params.pageId, message: String(req.body?.message || ""), imageUrl: req.body?.imageUrl ? String(req.body.imageUrl) : undefined, confirmationText: String(req.body?.confirmationText || ""), idempotencyKey: String(req.body?.idempotencyKey || "") });
  res.json({ success: true, data: result });
}));

export const callbackRouter = Router();
callbackRouter.get("/callback", asyncRoute(async (req, res) => {
  ...
  try {
    ...
    const pageCount = await completeAuthorization(code, stateValue, metaConfig());
...
function callbackHtml(origin: string, success: boolean, pageCount: number) {
  const payload = JSON.stringify(success ? { type: "META_CONNECTED", pageCount } : { type: "META_ERROR" });
